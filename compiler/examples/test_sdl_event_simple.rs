...
fn main() {
    let ir_code = std::fs::read_to_string("../examples/28-sdl2-event-simple.pole-ir")
        .expect("Failed to read IR file");

    println!("=== SDL2 Event Simple Demo ===\n");
    ...
    let mut codegen = CodeGen::new(&context, "sdl_event_simple", &arenas.codegen_arena);
    ...
    if output.status.success() {
        ...
        println!("\nNext step: Implement event loop with actual keyboard input");
    }
}
