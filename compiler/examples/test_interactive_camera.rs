...
fn main() {
    let ir_code = std::fs::read_to_string("../examples/31-interactive-camera.pole-ir")
        .expect("Failed to read IR file");

    println!("=== Interactive Camera Control Demo ===\n");
    ...
    let mut codegen = CodeGen::new(&context, "interactive_camera", &arenas.codegen_arena);
    ...
    let object_path = Path::new("/tmp/interactive_camera.o");
    target_machine
        .write_to_file(codegen.get_module(), FileType::Object, object_path)
        .expect("Failed to write object file");

    let exe_path = "/tmp/interactive_camera";
    ...
    println!("\n=== Running interactive demo (3 second test) ===\n");
    ...
    if output.status.success() {
        println!("\n✓✓✓ SUCCESS! Interactive camera works! ✓✓✓");
        println!("\nTo test interactively with real keyboard:");
        println!("  {}", exe_path);
        println!("\nControls:");
        println!("  WASD - Move camera around the isometric grid");
        println!("  ESC  - Quit the demo");
    }
}
