...
fn main() {
    let ir_source = fs::read_to_string("../examples/49-list-tilemap-modular.pole-ir")
        .expect("Failed to read modular tilemap IR file");
    ...
    let mut codegen = CodeGen::new(&context, "modular_tilemap", &arenas.codegen_arena);
    ...
    // Write object file
    let obj_path = Path::new("modular_tilemap.o");
    codegen
    ...
    let link_status = Command::new("cc")
        .args(&["-o", "modular_tilemap", "modular_tilemap.o"])
        .status()
    ...
    // Run the executable
    println!("\n=== Running executable ===");
    let output = Command::new("./modular_tilemap")
        .output()
    ...
    if exit_code == 0 {
        println!("\n✓ Modular tilemap test passed!");
    } else {
        println!("\n✗ Test failed");
    }

    // Cleanup
    let _ = fs::remove_file("modular_tilemap.o");
    let _ = fs::remove_file("modular_tilemap");
}
