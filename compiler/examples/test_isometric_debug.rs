...
fn main() {
    let ir_code = std::fs::read_to_string("../examples/27-isometric-debug.pole-ir")
        .expect("Failed to read IR file");

    println!("=== Compiling Isometric Debug Demo ===");
    
    let program = parse_ir(&ir_code).expect("Failed to parse IR");
    let arenas = CompilerArenas::new_default();
    let context = Context::create();
    let mut codegen = CodeGen::new(&context, "isometric_debug", &arenas.codegen_arena);
    ...
    let object_path = Path::new("/tmp/isometric_debug.o");
    target_machine
        .write_to_file(codegen.get_module(), FileType::Object, object_path)
        .expect("Failed to write object file");

    let exe_path = "/tmp/isometric_debug";
