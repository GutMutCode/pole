...
- [ ] **4.3 LLM Prompt 개선**
  - **문제**: LLM이 Pole IR 제약사항을 모름
  - **영향도**: 중간 - 생성 코드 품질 낮음
  - **산출물**: `src/pole/transformer/llm_transformer.py` 개선
  - **상세 작업**:
    - [ ] IR 문법 제약사항을 system prompt에 명시
      - 지원되는 연산자 목록
      - 주석 사용 불가 (임시)
      - 함수 정의 문법 (공백 필요)
      - 표현식 제약사항
    - [ ] 작동하는 IR 예제를 few-shot으로 제공
    - [ ] 실패 시 재시도 로직 추가
  - **테스트**: `tests/test_transformer.py`
  - **선행 조건**: 4.1, 4.2 완료 (또는 제약사항 명확화)
  - **우선순위**: P1
...
- [ ] **4.4 IR 후처리 자동화**
  ...
  - **상세 작업**:
    - [ ] LLM 생성 후 자동 파싱 검증
    - [ ] 파싱 실패 시 일반적인 오류 자동 수정
      - 주석 자동 제거
      - 공백 자동 추가
      - 괄호 자동 추가
    - [ ] 수정 불가능한 경우 LLM에게 재생성 요청
  - **테스트**: `tests/test_postprocessor.py` (신규)
  - **선행 조건**: 4.1 완료 (파싱 오류 패턴 파악)
  - **우선순위**: P1
...
| 작업 | 우선순위 | 예상 시간 | 의존성 | 영향도 |
|------|----------|-----------|--------|--------|
| 4.1 IR 파서 확장 | P0 | 4-6시간 | 없음 | 매우 높음 |
| 4.2 연산자 추가 | P0 | 1-2시간 | 없음 | 높음 |
| 4.3 LLM Prompt 개선 | P1 | 2-3시간 | 4.1, 4.2 | 중간 |
| 4.4 후처리 자동화 | P1 | 3-4시간 | 4.1 | 중간 |
| 4.5 에러 메시지 개선 | P2 | 2-3시간 | 4.1 | 낮음 |
| 4.6 IR 검증 도구 | P2 | 2-3시간 | 4.1, 4.2 | 낮음 |

**총 예상 시간**: 14-21시간

**최소 작업 (MVP)**: 4.1 + 4.2 (5-8시간) - LLM 생성 코드가 즉시 실행 가능
...
**다음 마일스톤**: Phase 4 P0 완료 → LLM 생성 코드가 수동 수정 없이 작동
